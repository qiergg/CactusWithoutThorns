package com.aebiz.sdk.UIKit.component.pulltorefresh;
...
import com.aebiz.sdk.UIKit.component.pulltorefresh.util.PtrLocalDisplay;
...
public class PtrFrameLayout extends ViewGroup {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mHeaderView != null) {
            ...
            if (DEBUG && DEBUG_LAYOUT) {
                Log.d(LOG_TAG, String.format("onMeasure header: height: %s, topMargin: %s, bottomMargin: %s, headerHeight: %s",
        ...
        if (mContent != null) {
            ...
            if (DEBUG && DEBUG_LAYOUT) {
                ...
                Log.d(LOG_TAG, String.format("onMeasure, mCurrentPos: %s, mLastPos: %s, top: %s",
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_CANCEL:
                ...
                if (mCurrentPos > POS_START) {
                    if (DEBUG) {
                        Log.d(LOG_TAG, String.format("call onRelease when user release"));
                    }
                    onRelease(false);
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (DEBUG) {
                    ...
                    Log.v(LOG_TAG, String.format("ACTION_MOVE: offsetY:%s, mCurrentPos: %s, moveUp: %s, canMoveUp: %s, moveDown: %s: canMoveDown: %s", offsetY, mCurrentPos, moveUp, canMoveUp, moveDown, canMoveDown));
                }
    ...
    private void updatePos() {
        ...
        if (mLastPos == POS_START && mCurrentPos != POS_START && mPtrUIHandlerHolder.hasHandler()) {
            if (mStatus == PTR_STATUS_INIT) {
                ...
                if (DEBUG) {
                    Log.i(LOG_TAG, String.format("PtrUIHandler: onUIRefreshPrepare, mAutoScrollRefreshTag %s", mAutoScrollRefreshTag));
                }
            }
    ...
    public void setRefreshCompleteHook(PtrUIHandlerHook hook) {
        ...
        hook.setResumeAction(new Runnable() {
            ...
            public void run() {
                if (DEBUG) {
                    Log.d(LOG_TAG, String.format("mRefreshCompleteHook resume."));
                }
    ...
    protected void onPtrScrollAbort() {
        if (mCurrentPos > 0 && mAutoScrollRefreshTag > 0) {
            if (DEBUG) {
                Log.d(LOG_TAG, String.format("call onRelease after scroll abort"));
            }
            onRelease(true);
        }
    }
    ...
    protected void onPtrScrollFinish() {
        if (mCurrentPos > 0 && mAutoScrollRefreshTag > 0) {
            if (DEBUG) {
                Log.d(LOG_TAG, String.format("call onRelease after scroll finish"));
            }
            onRelease(true);
        }
    }
    ...
    final public void refreshComplete() {
        ...
        if (delay <= 0) {
            if (DEBUG) {
                Log.d(LOG_TAG, String.format("performRefreshComplete at once"));
            }
    ...
    private void notifyUIRefreshComplete(boolean ignoreHook) {
        ...
        if (mCurrentPos != POS_START && !ignoreHook && mRefreshCompleteHook != null) {
            if (DEBUG) {
                Log.d(LOG_TAG, String.format("notifyUIRefreshComplete mRefreshCompleteHook run."));
            }
    ...
    private void sendCancelEvent() {
        if (DEBUG) {
            Log.d(LOG_TAG, String.format("send cancel event"));
        }
    ...
    private void sendDownEvent() {
        if (DEBUG) {
            Log.d(LOG_TAG, String.format("send down event"));
        }
    ...
    class ScrollChecker implements Runnable {
        ...
        private void finish() {
            if (DEBUG) {
                Log.v(LOG_TAG, String.format("finish, mCurrentPos:%s", mCurrentPos));
            }
            reset();
            onPtrScrollFinish();
        }
