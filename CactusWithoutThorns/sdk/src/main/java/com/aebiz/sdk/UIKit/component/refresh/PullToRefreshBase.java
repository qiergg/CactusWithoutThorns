...
//        }
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        // Styleables from XML
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.uikPullToRefresh);

        if (a.hasValue(R.styleable.uikPullToRefresh_uikptrMode)) {
            mMode = Mode.mapIntToValue(a.getInteger(R.styleable.uikPullToRefresh_uikptrMode, 0));
        }

        if (a.hasValue(R.styleable.uikPullToRefresh_uikptrAnimationStyle)) {
            mLoadingAnimationStyle = AnimationStyle.mapIntToValue(a.getInteger(
                    R.styleable.uikPullToRefresh_uikptrAnimationStyle, 0));
        }
         ...
         * Styleables from XML
         */
        if (a.hasValue(R.styleable.uikPullToRefresh_uikptrRefreshableViewBackground)) {
            Drawable background = a.getDrawable(R.styleable.uikPullToRefresh_uikptrRefreshableViewBackground);
            if (null != background) {
                mRefreshableView.setBackgroundDrawable(background);
            }
        } else if (a.hasValue(R.styleable.uikPullToRefresh_uikptrAdapterViewBackground)) {
            Utils.warnDeprecation("ptrAdapterViewBackground", "ptrRefreshableViewBackground");
            Drawable background = a.getDrawable(R.styleable.uikPullToRefresh_uikptrAdapterViewBackground);
            if (null != background) {
                mRefreshableView.setBackgroundDrawable(background);
            }
        }

        if (a.hasValue(R.styleable.uikPullToRefresh_uikptrOverScroll)) {
            mOverScrollEnabled = a.getBoolean(R.styleable.uikPullToRefresh_uikptrOverScroll, true);
        }

        if (a.hasValue(R.styleable.uikPullToRefresh_uikptrScrollingWhileRefreshingEnabled)) {
            mScrollingWhileRefreshingEnabled = a.getBoolean(
                    R.styleable.uikPullToRefresh_uikptrScrollingWhileRefreshingEnabled, false);
        }
